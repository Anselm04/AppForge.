...
recovery_regex='^(docs/DISASTER_RECOVERY\.md$|docs/OFFSITE_BACKUP\.md$|\.github/workflows/repository-backup\.yml$|\.github/workflows/repository-metadata-backup\.yml$|scripts/recovery-governance\.sh$)'
...
if [ -z "$recovery_updates" ]; then
  ...
  printf '%s\n' "$critical" >&2
  echo "Update the disaster/off-site recovery docs, a recovery workflow, or this governance policy in the same change." >&2
  exit 1
fi
