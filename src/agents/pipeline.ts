...
const SUPPORTED_TECH_STACKS = [
  ...
  "remix-node", "astro-node", "phaser-html5", "three-js-3d",
  "babylon-js-3d", "unity-webgl", "godot-html5", "react-native-game",
  "flutter-game", "ai-agent-python", "ai-agent-node", "openai-tool",
  "langchain-tool", "crewai-agent", "autogen-agent", "electron-react",
  "tauri-rust", "react-native-expo", "flutter-firebase", "capacitor-ionic",
  ...
  try {
    ...
    do {
      ...
      for (const task of tasks) {
        ...
        const coderLogId = await appendAgentLog({ projectId, agent: "Coder", content: `# ${task.module}\n`, isComplete: false });
        const fixContext = fixAttempt > 0 && validationResult ? `\n\nPREVIOUS BUILD FAILED WITH THESE ERRORS — FIX THEM:\n${validationResult.errors.slice(0, 5).join("\n")}` : "";
        let fileContent = "";
        await streamLLM([{ role: "system", content: `You are the Coder agent...${fixAttempt > 0 ? " FIX RETRY" : ""}${localeHint}` }, { role: "user", content: `App: ${appTitle}\nModule: ${task.module}\nTask: ${task.description}\nTech stack: ${techStack}${fixContext}` }], (chunk) => { fileContent += chunk; emit("Coder", "chunk", { module: task.module, text: chunk }); }, signal, "coder");
    ...
    const filesSummary = Object.entries(generatedFiles).map(([name]) => `- ${name}`).join("\n");
    const reviewContext = validationResult ? `\nValidation result: ${validationResult.passed ? "PASSED" : "FAILED"}\nStage: ${validationResult.stage}\nErrors: ${validationResult.errors.slice(0, 5).join("; ")}` : "";
    let reviewOutput = "";
    await streamLLM([{ role: "system", content: `You are the Reviewer agent...${localeHint}` }, { role: "user", content: `App: ${appTitle}\nGenerated files:\n${filesSummary}\nTech stack: ${techStack}${reviewContext}` }], (chunk) => { reviewOutput += chunk; emit("Reviewer", "chunk", { text: chunk }); }, signal, "reviewer");
    ...
    const estBundleKB = Math.round(Object.values(generatedFiles).filter((f) => f.endsWith(".ts") || f.endsWith(".tsx") || f.endsWith(".js")).join("").length / 3 / 1024);
    const costReport = estimateLicenseAndCost(parsedDeps, techStack, estBundleKB, true);
    const snapshotId = await createBuildSnapshot({ projectId, userId, version: nextVersion, label: appTitle ? `v${nextVersion} — ${appTitle}` : `v${nextVersion}`, files: generatedFiles, fileCount: Object.keys(generatedFiles).length, techStack, validationResult, auditScores: null, costEstimate: costReport });
