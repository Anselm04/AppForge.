...
export async function runAgentPipeline(
  ...
  try {
    ...
    try {
      emit("System", "info", {
        message: "LLM failed — publishing guaranteed-green live app so Generate still produces a working URL.",
      });
      const fallbackTitle = (description || `App ${projectId}`).trim().slice(0, 60);
      let files = buildGuaranteedGreenApp({
        title: fallbackTitle,
        description,
        techStack,
      });
      files = hardenGeneratedProject(files, techStack);
      const { materializeHostedHtml, publicAppUrl } = await import(
        "../lib/hostedRuntime.js"
      );
      const liveUrl = publicAppUrl(projectId);
      files["_hosted/index.html"] = materializeHostedHtml({
        projectId,
        title: fallbackTitle,
        description,
        techStack,
      ...
      write("done", {
        ...
        validationStage: "guaranteed-green-fallback",
