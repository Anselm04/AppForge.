...
export async function runAgentPipeline(
  ...
  try {
    ...
    let requirementManifest: RequirementManifest | null =
      projectRow?.requirementManifest
        ? validateRequirementManifest(projectRow.requirementManifest)
        : null;
    ...
    outerLoop: while (!signal?.aborted) {
      ...
      requirementManifest = createRequirementManifest({
        productContract,
        productPlan,
        previous: requirementManifest,
      });
      ...
      do {
        if (fixAttempt > 0) {
          ...
          await recordCoordinationEvent({
            agent: "Coder",
            type: "decision",
            detail: `Surgical repair attempt ${fixAttempt} persisted ${Object.keys(patches).length} parsed patch file(s) without changing the canonical product scope.`,
            provider: provider.id,
            model: coderModel || provider.defaultModel,
          });
          emit("Coder", "complete", {
          ...
          for (const task of [...tasks].sort(
              ...
              if (coordinationRecord?.taskStates[task.id]) {
                coordinationRecord = updateTaskCoordinationState(
                  ...
                  task.id,
                  {
                    status: "paused",
                    lastError:
                      "Agent execution interrupted by abort/timeout signal",
                  },
                );
                await recordCoordinationEvent({
                  agent: task.agent,
            ...
            if (existingTaskState?.status === "completed") {
              emit("Coder", "task_resume_skip", {
                ...
                message:
                  "Skipping already completed task with persisted output.",
              });
              continue;
            }
            ...
            for (let taskAttempt = 1; taskAttempt <= 3; taskAttempt++) {
              ...
              try {
                ...
                await recordCoordinationEvent({
                  ...
                  provider: attemptProvider.id,
                  model: coderModel || attemptProvider.defaultModel,
                  detail: `Task completed with owned files: ${outputFiles.join(", ")}`,
                });
                await recordCoordinationEvent({
                  agent: task.agent,
                  type: "handoff",
        ...
        if (!requirementManifest) {
          throw new Error(
            "Requirement manifest was not initialized by the Planner",
          );
        }
        ...
        await updateProjectRequirementManifest(projectId, requirementManifest);
        await updateProjectFiles(projectId, { ...generatedFiles });
        emit("System", "requirements_updated", {
          revision: requirementManifest.revision,
          unresolvedMustHaveIds: requirementManifest.unresolvedMustHaveIds,
          requirements: requirementManifest.requirements.map((requirement) => ({
            id: requirement.id,
        ...
        if (validationMode === "full") {
          ...
          generatedFiles["appforge.requirements.json"] =
            serializeRequirementManifest(requirementManifest);
          await updateProjectRequirementManifest(
            projectId,
            requirementManifest,
          );
          ...
          await recordCoordinationEvent({
            agent: "Testing",
            type: "output",
            detail: `Generated ${Object.keys(testFiles).length} requirement-linked test file(s) from the coordinated plan context.`,
          });
        ...
        await recordCoordinationEvent({
          ...
          detail: `Validation stage ${validationResult.stage}: ${validationResult.passed ? "passed" : validationResult.errors.slice(0, 3).join("; ")}`,
        });
