...
export function Admin() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white p-8">
      ...
      <p className="text-slate-500 dark:text-slate-400 mb-8">Signed in as {me.email}</p>

      <div className="flex gap-2 mb-8">
        {(["analytics", "codes"] as const).map((item) => (
          <button
            key={item}
            type="button"
            onClick={() => setTab(item)}
            className={`px-4 py-2 rounded-lg font-semibold ${
              tab === item ? "bg-blue-600 text-white" : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300"
            }`}
          >
            {item === "analytics" ? "Analytics" : "God codes"}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              { label: "Signups (7d)", value: analytics.counts.recentSignups7d },
              { label: "Builds started", value: analytics.counts.buildsStarted },
              { label: "Builds (30d)", value: analytics.counts.builds30d },
              { label: "Active subs", value: analytics.counts.activeSubscriptions },
              { label: "Credits held", value: analytics.counts.creditBalanceSum },
            ].map((card) => (
              <div key={card.label} className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow">
          ...
          <table className="w-full bg-white dark:bg-slate-800 rounded-xl shadow overflow-hidden">
            ...
            <tbody>
              {analytics.subscriptionsByTier.map((row) => (
                <tr key={`${row.tier}-${row.status}`} className="border-t border-slate-200 dark:border-slate-700">
              ...
              <tbody>
                {analytics.recentUsers.map((u) => (
                  <tr key={u.id} className="border-t border-slate-200 dark:border-slate-700">
                    ...
                    <td className="p-3">{u.unlimited ? "Unlimited" : u.credits}</td>
                    <td className="p-3 capitalize">{u.tier}</td>
                    <td className="p-3">{u.subscriptionStatus ? `${u.subscriptionTier} (${u.subscriptionStatus})` : "—"}</td>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow max-w-xl">
            ...
            <p className="text-sm text-slate-500 mb-4">
              The plaintext code is shown once. Copy it now — it cannot be listed again.
            </p>
            <div className="flex flex-col gap-3 mb-4">
              <label className="text-sm font-semibold">
                ...
                <select
                  value={grantType}
                  onChange={(e) => setGrantType(e.target.value as "lifetime" | "limited")}
              ...
              {createCode.isError && (
                <p className="text-sm text-amber-700">{String((createCode.error as Error)?.message || "")}</p>
              )}
            </div>
            {minted && (
              <div className="border border-amber-300 bg-amber-50 dark:bg-amber-950/40 dark:border-amber-800 rounded-lg p-4">
                <p className="text-sm font-semibold mb-2">Copy now — this is the only time it is shown.</p>
                <code className="block text-lg font-mono break-all mb-3">{minted}</code>
                <button
                  type="button"
                  onClick={async () => {
                    try {
              ...
              <tbody>
                {codes.map((c) => (
                  <tr key={c.id} className="border-t border-slate-200 dark:border-slate-700">
                    ...
                    <td className="p-3">{c.grantType === "lifetime" ? "Unlimited" : c.credits}</td>
