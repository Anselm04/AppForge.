...
export function Redeem() {
  ...
  const redeem = useMutation({
    mutationFn: () => trpc.admin.redeemCode.mutate({ code: code.trim() }),
    onSuccess: (data) => {
      setMessage(
        data.unlimited
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800">
        <div className="max-w-md mx-auto px-4 py-20 text-center">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">Redeem a code</h1>
          <p className="text-slate-600 dark:text-slate-300 mb-6">Sign in to redeem a code on your account.</p>
          <Link
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800">
      <div className="max-w-md mx-auto px-4 py-20">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2 text-center">Redeem a code</h1>
        <p className="text-slate-600 dark:text-slate-300 mb-8 text-center">
          Enter the one-time code you were given. Each code can be used once, on this account only.
        </p>
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label htmlFor="redeem-code" className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">
                Code
              </label>
              <input
                id="redeem-code"
