...
import { z } from "zod";
import { getGithubConnection, getProjectById } from "../db.js";
  ...
  pushToRepo: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!createRes.ok && createRes.status !== 422) {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `GitHub repo creation failed: ${err}` });
      }

      const repoData = (createRes.status === 422
        ? { full_name: `${conn.githubUsername}/${input.repoName}` }
        : await createRes.json()) as { full_name: string };

      // Push files via Contents API
      for (const [path, content] of Object.entries(files)) {
        const encoded = Buffer.from(content).toString("base64");
        await fetch(`https://api.github.com/repos/${repoData.full_name}/contents/${path}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${conn.accessToken}`,
            "Content-Type": "application/json",
