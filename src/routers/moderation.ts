...
export async function moderateUserContent(userId: number, text: string, projectId?: number): Promise<{ allowed: boolean; reason?: string }> {
  ...
  if (result) {
    ...
    if (strikeCount >= 3) {
      await db
        .update(schema.users)
        .set({ isBanned: true, bannedAt: new Date(), banReason: `3 strikes: ${result.category}` })
        .where(eq(schema.users.id, userId));
      await db.insert(schema.complianceRecords).values({
        recordType: "security_incident",
        userId,
        details: { action: "auto_ban_3_strikes", category: result.category, text: text.slice(0, 200) },
