...
type Props = { busy?: boolean; onChoose: (plan: AppForgePlan) => void };
...
export function Pricing({ busy, onChoose }: Props) {
  return <section><h2>Choose your AppForge plan</h2><p>Build credits are used for complete AI-assisted build attempts.</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 16 }}>{tiers.map(t => <article key={t.plan} style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: 24 }}><h3>{t.name}</h3><p style={{ fontSize: 28, fontWeight: 800 }}>{t.price}<small style={{ fontSize: 14 }}>/month</small></p><p><strong>{t.credits}</strong></p><p>{t.description}</p><button disabled={busy} onClick={() => onChoose(t.plan)}>Choose {t.name}</button></article>)}</div></section>;
}
