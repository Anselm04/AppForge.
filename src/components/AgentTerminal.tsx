...
export function AgentTerminal({ projectId, enabled = true }: Props) {
  ...
  const runCommand = (cmd: string) => {
    ...
    setLines((prev) => [
      ...prev,
      { id: nextId.current++, text: `$ ${trimmed}`, kind: "cmd" },
    ]);
    if (trimmed === "help") {
      setLines((prev) => [
        ...prev,
        {
          id: nextId.current++,
          text: "Commands: help, logs, validate, clear — full shell runs in deployed sandbox.",
          kind: "info",
        },
      ]);
      ...
      setLines((prev) => [
        ...
        {
          id: nextId.current++,
          text: `${logs?.length ?? 0} agent log entries (auto-streamed above)`,
          kind: "out",
        },
      ]);
      ...
      setLines((prev) => [
        ...
        {
          id: nextId.current++,
          text: "Use Code tab → Re-run validator on the active file.",
          kind: "info",
        },
      ]);
    } else if (trimmed === "clear") {
      setLines([
        { id: nextId.current++, text: "Terminal cleared", kind: "info" },
      ]);
    } else {
      setLines((prev) => [
        ...prev,
        {
          id: nextId.current++,
          text: `Sandbox exec not available in browser — use Senior Dev Chat for autonomous edits.`,
          kind: "err",
        },
      ]);
    }
