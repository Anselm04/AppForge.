...
describe("Stripe webhook provisioning guards", () => {
  ...
  it("only provisions configured credit-pack sizes after payment", () => {
    expect(source).toContain("const CREDIT_PACK_SET = new Set<number>(CREDIT_PACKS)");
    expect(source).toContain("session.payment_status === \"paid\"");
    expect(source).toContain("Unrecognized Stripe credit pack metadata");
  });
