...
export function verifyPreviewSignature(projectId: number, sig: string | undefined): boolean {
  if (!sig) return false;
...
export async function deployToVercel(
  ...
  const name = projectName.toLowerCase().replace(/[^a-z0-9-]/g, "-").slice(0, 30);
  ...
  if (!res.ok)
    throw new Error(`Vercel deploy creation failed: ${res.status} ${await res.text()}`);
...
async function deployToNetlify(
  ...
  if (!siteId) {
    ...
    if (!r.ok) throw new Error(`Netlify site create failed: ${await r.text()}`);
  ...
  const d = (await r.json()) as { ssl_url?: string; deploy_ssl_url?: string; url?: string };
  return d.ssl_url || d.deploy_ssl_url || d.url || `https://app.netlify.com/sites/${siteId}`;
}

function customerFlyAppName(projectName: string): string {
  const slug =
    projectName
      .toLowerCase()
      .replace(/[^a-z0-9-]/g, "-")
...
async function ensureFlyApp(appName: string, token: string): Promise<void> {
  ...
  if (!r.ok && r.status !== 422 && r.status !== 409) {
    ...
    if (!g.ok) throw new Error(`Fly app create failed: ${r.status} ${await r.text()}`);
  }
}
...
async function flyctlAvailable(): Promise<boolean> {
  return (
    ...
    (await runCmd("fly", ["version"], process.cwd(), 10000)).exitCode === 0
  );
}
...
async function deployToFly(
  ...
  const appName = customerFlyAppName(projectName);
  ...
  if (protectedNames.has(appName))
    throw new Error("Refusing to deploy customer project over the AppForge production application");
  if (!files["Dockerfile"])
  ...
  if (await flyctlAvailable()) {
    const dir = join(tmpdir(), `appforge-fly-${Date.now()}`);
    try {
      ...
      const bin =
        (await runCmd("flyctl", ["version"], process.cwd(), 5000)).exitCode === 0
          ? "flyctl"
          : "fly";
      const safeEnv = {
      ...
      if (d.exitCode !== 0)
        throw new Error(`fly deploy failed: ${(d.stderr || d.stdout).slice(0, 500)}`);
  ...
  return {
    ...
    note: `Fly app ${appName} created; install flyctl on the build host to complete automated deployment.`,
  };
}
...
export function listDeployDestinations(): DeployDestinationStatus {
  return {
    ...
    netlify: { configured: !!process.env.NETLIFY_AUTH_TOKEN, label: "Netlify" },
...
export async function deployProject(opts: {
  ...
  switch (destination) {
    ...
    case "fly": {
      const r = await deployToFly(projectName, { ...files });
