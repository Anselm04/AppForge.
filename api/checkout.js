...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user?.id || !user.email || !price) {
      return res
        .status(401)
        .json({ error: "Sign in and choose a valid plan." });
    }
    ...
    if (!appUrl || !process.env.STRIPE_SECRET_KEY) {
      return res
        .status(503)
        .json({ error: "Checkout is not configured yet." });
    }
    ...
    if (!response.ok || !session?.url) {
      console.error("serverless_checkout_failed", {
    ...
    console.error("serverless_checkout_unhandled", {
